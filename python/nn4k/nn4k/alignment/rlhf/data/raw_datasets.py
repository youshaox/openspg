...
# Local dataset
class LocalFileDataset(PromptRawDataset):

    def __init__(self, raw_datasets, dataset_name=None, output_path="/tmp/rlhf/data", seed=None, local_rank=None):
        """
        Args:
            raw_datasets(str or Any): An instance returned by load_dataset. Or a str to local file.
        """
        import os
        if not os.path.exists(output_path):
            try:
                ...
                print(f"Failed to mkdir {output_path}", e)
        ...
        if isinstance(raw_datasets, str):
            self.raw_datasets = load_dataset(raw_datasets)
            self.dataset_name = dataset_name or raw_datasets
        else:
            self.raw_datasets = raw_datasets
            self.dataset_name = dataset_name or "local"

        self.dataset_name_clean = dataset_name.replace("/", "_")

    def get_train_data(self):
        """下述接口，需要根据raw_datasets实际情况进行构造，这里仅提供一个例子"""
        return self.raw_datasets["train"]
