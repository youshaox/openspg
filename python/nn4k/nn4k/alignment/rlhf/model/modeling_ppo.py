...
from alignment.rlhf.model.model_utils import (
    hf_get_decoder,
...
from alignment.rlhf.utils.constants import CHAT_GLM_ARCH

"""
copy from trlx, we support trlx models in deepspeed
"""
...
class AutoModelForCausalLMWithValueHead(PreTrainedModelWrapper):
    ...
    def __init__(
        ...
        self.is_chat_glm = self.base_model.config.architectures[0] == CHAT_GLM_ARCH
    ...
    def forward(
        ...
        if self.is_chat_glm:
            # ChatGLM 需要交换一下seq_len和bs的维度
            hidden_states = hidden_states.clone().detach().transpose(1, 0)
    ...
    def load_state_dict(self, state_dict, strict: bool = True):
        """
        Override the load_state_dict method. Add "base_model." prefix to base_model's weight.
        """
        print("AutoModelForCausalLMWithValueHead # load_state_dict")
        print(f"state_dict.keys():  {state_dict.keys()}")
        v_head_keys = set()
        formatted_state_dict = {}
        for key in state_dict.keys():
            if key.startswith("v_head."):
                v_head_keys.update(key)
                formatted_state_dict.update({key: state_dict[key]})

        not_v_head_keys = state_dict.keys() - v_head_keys
        for key in not_v_head_keys:
            if key.startswith("base_model."):
                formatted_state_dict.update({
                    key: state_dict[key]
                })
            else:
                formatted_state_dict.update({
                    f"base_model.{key}": state_dict[key]
                })
        return super(AutoModelForCausalLMWithValueHead, self).load_state_dict(formatted_state_dict)
...
class AutoModelForCausalLMWithHydraValueHead(AutoModelForCausalLMWithValueHead):
    ...
    def __init__(
        ...
        if self.num_layers_unfrozen > 0:
            ...
            branch_model = branch_class(
                self.base_model,
                num_layers_unfrozen=self.num_layers_unfrozen,
            )
            branch_model.make_head()
            self.frozen_head = branch_model.eval()
    ...
    def forward_hydra_with_value(
        ...
        hydra_outputs = self.forward_hydra(input_ids=input_ids,
                                           attention_mask=attention_mask,
                                           position_ids=position_ids,
                                           past_key_values=past_key_values,
                                           head_mask=head_mask,
                                           ...
                                           output_hidden_states=output_hidden_states,
                                           return_dict=True, )
        value = self.frozen_head.forward_head(hydra_outputs.hidden_states[-1])
        if not return_dict:
            return (hydra_outputs.logits, value)
        else:
            hydra_outputs.value = value
            return hydra_outputs

    def load_state_dict(self, state_dict, strict: bool = True):
        """
        Override the load_state_dict method.
        Only load state_dict to the base_model and v_head, no care about frozen_head.
        """
        v_head_state_dict = {}
        not_v_head_state_dict = {}
        for key in state_dict.keys():
            if key.startswith("v_head."):
                v_head_state_dict.update({key[len("v_head."):]: state_dict[key]})
            else:
                not_v_head_state_dict.update({key, state_dict[key]})

        self.v_head.load_state_dict(v_head_state_dict, strict)
        return self.base_model.load_state_dict(not_v_head_state_dict, strict)
...
class ModelBranch(transformers.PreTrainedModel):
    ...
    def __init__(
        ...
        all_decoder_blocks = list(hf_get_decoder_blocks(base_model))
        self.layer_size = len(all_decoder_blocks)
        decoder_blocks = all_decoder_blocks[-num_layers_unfrozen:]
        final_norm = hf_get_decoder_final_norm(base_model)
        lm_head = hf_get_lm_head(base_model)
        params = []
        [params.extend(item.parameters()) for item in decoder_blocks]
        params.extend(final_norm.parameters())
        params.extend(lm_head.parameters())
        # for zero3, 需gather回参数。暂不convert to ds param
        with GatheredParameters(params):
            self.decoder_blocks = nn.ModuleList([deepcopy(item) for item in decoder_blocks])
    ...
    def make_head(self):
        ...
        for parameter in self.v_head.parameters():
            parameter.requires_grad_(False)

    def forward_head(self, hidden_states):
        return self.v_head(hidden_states).squeeze(-1) if self.v_head else None
...
class ChatGLMModelBranch(ModelBranch):

    def __init__(self,
                 base_model: transformers.PreTrainedModel,
                 *,
                 num_layers_unfrozen: int):
        super(ChatGLMModelBranch, self).__init__(base_model, num_layers_unfrozen=num_layers_unfrozen)
        self._base_model = base_model

    # ref to https://huggingface.co/THUDM/chatglm-6b/blob/main/modeling_chatglm.py
    def forward(
            self,
            hidden_states: torch.Tensor,  # Takes as input hidden_states instead of input_ids
            output_shape: torch.Tensor,  # output_size given by main trunk
        ...
        def get_prompt(instance, batch_size, device, dtype=torch.half):
            prefix_tokens = instance.prefix_tokens.unsqueeze(0).expand(batch_size, -1).to(device)
            past_key_values = instance.prefix_encoder(prefix_tokens).type(dtype)
            past_key_values = past_key_values.view(
                batch_size,
                instance.pre_seq_len,
                instance.num_layers * 2,
                instance.num_attention_heads,
                instance.hidden_size // instance.num_attention_heads
            )
            # seq_len, b, nh, hidden_size
            past_key_values = instance.dropout(past_key_values)
            past_key_values = past_key_values.permute([2, 1, 0, 3, 4]).split(2)
            # past_key_values = [(v[0], v[1]) for v in past_key_values]
            return past_key_values

        pre_seq_len = self._base_model.transformer.pre_seq_len
        seq_len = hidden_states.shape[0]  # = input_ids.size(-1)
        batch_size = hidden_states.shape[1]
        if past_key_values is None:
            if pre_seq_len is not None:
                past_key_values = get_prompt(self._base_model.transformer, batch_size=batch_size,
                                             device=hidden_states.device, dtype=hidden_states.dtype)
            else:
                past_key_values = tuple([None] * self.layer_size)

        if pre_seq_len is not None and attention_mask is not None:
            prefix_attention_mask = torch.ones(batch_size, 1, seq_len, pre_seq_len).to(
        ...
        all_hidden_states = () if output_hidden_states else None
        if attention_mask is None:
            # device need to set input_ids.device?
            attention_mask = torch.zeros(1, 1, device=hidden_states.device).bool()
        else:
            attention_mask = attention_mask.to(hidden_states.device)
        ...
        for i, layer in enumerate(self.decoder_blocks):
            ...
            layer_id = self.layer_size - len(self.decoder_blocks) + i
            layer_past = past_key_values[layer_id]
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_ret = layer(
                    ...
                    position_ids=position_ids,
                    attention_mask=attention_mask,
                    layer_id=torch.tensor(layer_id),
                    layer_past=layer_past,
                    use_cache=use_cache,
                    output_attentions=output_attentions
                )
        ...
        if not return_dict:
            return tuple(v for v in [lm_logits, hidden_states, presents, all_hidden_states, all_self_attentions]
                         if v is not None)
...
class AutoModelForSeq2SeqLMWithHydraValueHead(AutoModelForSeq2SeqLMWithValueHead):
    ...
    def forward_hydra_with_value(
        ...
        hydra_outputs = self.forward_hydra(input_ids=input_ids,
                                           ...
                                           output_hidden_states=output_hidden_states,
                                           return_dict=True, )
        value = self.frozen_head.forward_head(hydra_outputs.decoder_hidden_states[-1])
        if not return_dict:
            return (hydra_outputs.logits, value)
        else:
            hydra_outputs.value = value
...
def hf_get_branch_class(
    ...
    glm_branch_supported_archs = ["ChatGLMModel"]
