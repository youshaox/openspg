...
class LinearLayer_LoRA(nn.Module):
    ...
    def eval(self):
        ...
        # for zero3 trace, 使用sub_module.training 来trace module
        self.training = False
...
def convert_linear_layer_to_lora(model,
    ...
    assert len(replace_name) > 0, f"No module name matches {part_module_name}"
...
def convert_linear_layer_to_lora_list(model,
                                 part_module_name_list,
                                 lora_dim=0,
                                 lora_scaling=1,
                                 lora_dropout=0):
    replace_name = []
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            ...
            match_linear = False
            for part_module_name in part_module_name_list:
                if part_module_name in name:
                    match_linear = True
            if match_linear:
                replace_name.append(name)
    assert len(replace_name) > 0, f"No module name matches {part_module_name_list}"
    for name in replace_name:
        module = recursive_getattr(model, name)
        tmp = LinearLayer_LoRA(
            module.weight, lora_dim, lora_scaling, lora_dropout,
